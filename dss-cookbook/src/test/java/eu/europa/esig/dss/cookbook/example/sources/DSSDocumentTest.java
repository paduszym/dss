...
class DSSDocumentTest {
	...
	void testDOMDocument() {
		Document document = DomUtils.buildDOM("<el id=\"signedData\">Text</el>");
		...
		assertNotNull(domDocument.getDigestValue(DigestAlgorithm.SHA256));

		try (InputStream is = domDocument.openStream()) {
			// expected behavior
		} catch (UnsupportedOperationException | IOException e) {
			fail("Cannot open an InMemoryDocument");
		}
	...
	void testCMSDocument() throws CMSException {
		...
		assertNotNull(cmsDocument.getDigestValue(DigestAlgorithm.SHA256));
